...
export default class ContactsTable extends React.Component {
  ...
  deleteButton(id) {
    var contacts = this.props.contacts.map((contact) => {
      return (
        <button type="button" onClick={this.handleDelete.bind(this, contact.id)}>
            Delete
        </button>
        )
      });
  ...
  render() {
    ...
    return (
      <div>
        ...
        <BootstrapTable handleDelete={ this.handleDelete.bind(this) } ref='table' data={ this.props.contacts } deleteRow={ true } selectRow={ selectRowProp } options={ options }>
          ...
          <TableHeaderColumn width='100px' dataField='button' dataFormat={this.deleteButton.bind(this)}>Delete</TableHeaderColumn>
        </BootstrapTable>
      </div>
    )
  }
